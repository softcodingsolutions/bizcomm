module Api
    class TokensController < Devise.api.config.base_controller.constantize
        ...
        before_action :check_user_status, only: [:sign_in]
        ...
        def check_user_status
            render json: { error: "You are not allowed to sign in" }, status: :unauthorized if resource_class.find_by_email(params[:email]).rejected? 
        end
    end
end
